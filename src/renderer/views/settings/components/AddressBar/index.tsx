...
const DynamicSecurityButtonToggle = observer(() => {
  const { showDynamicSecurityButton } = store.settings as any;

  return (
    <Row onClick={onSwitchChange('showDynamicSecurityButton')}>
      <Title>Show dynamic security button</Title>
      <Control>
        <Switch value={!!showDynamicSecurityButton} />
      </Control>
    </Row>
  );
});
