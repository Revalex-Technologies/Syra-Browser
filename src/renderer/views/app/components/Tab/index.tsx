...
const Content = observer(({ tab }: { tab: ITab }) => {
  return (
    <StyledContent>
      ...
      {!tab.isPinned && (
        <StyledTitle
          isIcon={tab.favicon !== '' || tab.loading}
...
export default observer(({ tab }: { tab: ITab }) => {
  const defaultColor = store.theme['tab.inactive.background'];

  const defaultHoverColor = store.theme['tab.inactive.hover.background'];

  const defaultSelectedHoverColor =
    store.theme['tab.selected.hover.background'];
